...
fn one_big_uint() -> BigUint { BigUint::new(vec![1])}

fn one() -> Noun {Noun::Atom(one_big_uint())}
...
fn fas(addr: Atom, noun: Rc<Noun>) -> Rc<Noun> {
    ...
    match addr_iter.len() {
        ...
        _ => {
            let rest = fas(&addr >> 1u32, noun);
            fas_u32(if (addr & one_big_uint()) == one_big_uint() {3} else {2}, rest)}
        ...
        n => 
            match (*noun).clone() {
                ...
                Noun::Cell(p, q) =>  match n {
                    ...
                    3 => q,
                    _ => {
                        let rest = fas_u32(n >> 1, Rc::new(Noun::Cell(p, q)));
    ...
    match addr_iter.len() {
        ...
        _ => {
            ...
            if (&addr & one_big_uint()) == one_big_uint() {
                hax(a, cell(fas(addr - 1u32, target.clone()), target.clone()), target.clone())
            } else {
                hax(a, Rc::new(Noun::Cell(new_value, fas(addr + 1u32, target.clone()))), target)
            }
        }
    }
}
...
fn tar_u32(subj: Rc<Noun>, op: u32, formula: Rc<Noun>) -> Rc<Noun> {
    match op {
       0 => {
           let Noun::Atom(b) = (*formula).clone() else {panic!()};
           fas(b, subj)
       },
       1 => formula,
       2 => {
           let Noun::Cell(b, c) = (*formula).clone() else {panic!()};
       ...
       6 => {
           let Noun::Cell(b, c) = (*formula).clone() else {panic!()};
           let Noun::Cell(c, d) = (*c).clone() else {panic!()};

           tar(subj.clone(), tar_u32(cell(c, d), 0, tar_u32(cell(atom_ref(2), atom_ref(3)),0 , tar_u32(subj, 4, cell(atom_ref(4), b)))))
       },
       7 => {
           let Noun::Cell(b, c) = (*formula).clone() else {panic!()};
           tar(tar(subj, b), c)
       ...
       9 => {
           let Noun::Cell(b, c) = (*formula).clone() else {panic!()};
           tar_u32(tar(subj, c), 2, cell(cell(Rc::new(Noun::SIG), Rc::new(one())), cell(Rc::new(Noun::SIG), b)))
       },
       10 => {
           let Noun::Cell(b, d) = (*formula).clone() else {panic!()};
           let Noun::Cell(b, c) = (*b).clone() else {panic!()};
           let Noun::Atom(b) = (*b).clone() else {panic!()};
       ...
       11 => {
           ...
           match (*b).clone() {
               ...
               Noun::Cell(_, c) => tar_u32(cell(tar(subj.clone(), c), tar(subj, d)), 0, atom_ref(3))
           }
       }
       _ => panic!()
    }
...
pub fn tar(subj: Rc<Noun>, formula: Rc<Noun>) -> Rc<Noun> {
    let Noun::Cell(op, formula) = (*formula).clone() else {panic!()};
    match (*op).clone() {
        Noun::Cell(_, _) => {
            Rc::new(Noun::Cell(tar(subj.clone(), op), tar(subj, formula)))
        }
        Noun::Atom(op) => {
...
pub fn eval_gate(gate: Rc<Noun>) -> Rc<Noun> {
    tar(gate, cell(atom_ref(9), cell(atom_ref(2), cell(Rc::new(Noun::SIG), atom_ref(1)))))
}
