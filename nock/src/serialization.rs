...
pub fn cue_bytes(bytes: &[u8]) -> Rc<Noun> {
    cue_reader(&mut LittleEndianReader::new(bytes), &mut 0, &mut BTreeMap::new())
}

fn cue_reader(reader: &mut LittleEndianReader, offset: &mut u32, refs: &mut BTreeMap<u32, Rc<Noun>> ) -> Rc<Noun> {    
...
fn read_bits(reader: &mut LittleEndianReader, bits: u64) -> Atom {
...

    {
        let mut digit = 0u32;
        for shift in 0..bits {
            digit = digit | (if reader.read_bit().unwrap() {1 << shift} else {0});
        }
        v.push(digit);
    }
